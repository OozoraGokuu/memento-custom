...
QCoro::Task<QVariantMap> AnkiClient::noteAddableAsync(const Term *term)
{
    ...
    QJsonObject params = co_await QtConcurrent::run(
        [&profile = *profileCopy, &term = *termCopy] () -> QJsonObject
        {
            QJsonArray notes;
            ...
            if (!term.reading().isEmpty())
            {
                ...
                ctx = Anki::Note::build(profile, term, false);
                notes.append(std::move(ctx.ankiObject));
            }
...
QCoro::Task<QVariantMap> AnkiClient::noteAddableAsync(const Kanji *kanji)
{
    ...
    QJsonObject params = co_await QtConcurrent::run(
        [&profile = *profileCopy, &kanji = *kanjiCopy] () -> QJsonObject
        {
            QJsonArray notes;

            Anki::Note::Context ctx =
                Anki::Note::build(profile, kanji, false);
            notes.append(std::move(ctx.ankiObject));
...
QCoro::Task<QVariantMap> AnkiClient::addNoteAsync(const Term *term)
{
    ...
    const Anki::Note::MediaSnapshot mediaSnapshot =
        Anki::Note::captureMedia(*m_context);
    populate(termCopy.get(), &mediaSnapshot);
    Anki::Note::Context ctx = co_await QtConcurrent::run(
        [&profile = *profileCopy, &term = *termCopy, mediaSnapshot]
        () -> Anki::Note::Context
        {
            return Anki::Note::build(
                profile, term, true, mediaSnapshot);
        }
    );
...
QCoro::Task<QVariantMap> AnkiClient::addNoteAsync(const Kanji *kanji)
{
    ...
    const Anki::Note::MediaSnapshot mediaSnapshot =
        Anki::Note::captureMedia(*m_context);
    populate(kanjiCopy.get(), &mediaSnapshot);
    Anki::Note::Context ctx = co_await QtConcurrent::run(
        [&profile = *profileCopy, &kanji = *kanjiCopy, mediaSnapshot]
        () -> Anki::Note::Context
        {
            return Anki::Note::build(
                profile, kanji, true, mediaSnapshot);
        }
    );
    co_return co_await addNoteHelper(profileCopy.get(), std::move(ctx));
}
...
QCoro::Task<QVariantMap> AnkiClient::openDuplicatesAsync(const Term *term)
{
    ...
    Anki::Note::Context ctx = co_await QtConcurrent::run(
        [&profile = *profileCopy, &term = *termCopy]
        () -> Anki::Note::Context
        {
            return Anki::Note::build(profile, term, false);
        }
    );
    QString fieldValue =
...
QCoro::Task<QVariantMap> AnkiClient::openDuplicatesAsync(const Kanji *kanji)
{
    ...
    Anki::Note::Context ctx = co_await QtConcurrent::run(
        [&profile = *profileCopy, &kanji = *kanjiCopy]
        () -> Anki::Note::Context
        {
            return Anki::Note::build(profile, kanji, false);
        }
    );
    QString fieldValue =
        ctx.ankiObject[AnkiConnect::Note::FIELDS][fieldKey].toString();
...
void AnkiClient::populate(
    Expression *expression,
    const Anki::Note::MediaSnapshot *mediaSnapshot) const
{
    ...
    const bool useNativeSecondary =
        secondaryList != nullptr && secondaryList->fullTimelineReady();

    expression->setTitle(
        mediaSnapshot != nullptr ? mediaSnapshot->title : state->title());
    QString subtitleText = mediaSnapshot != nullptr ?
        mediaSnapshot->primaryText :
        (useNativePrimary ? primaryList->activeText() :
                           state->subtitle()->text());
    expression->setSubtitle(subtitleText.remove(m_subtitleFilterRegex));
    const double primaryDelay = mediaSnapshot != nullptr ?
        mediaSnapshot->primaryTrack.delay : state->subtitle()->delay();
    expression->setStartTime((mediaSnapshot != nullptr ?
        mediaSnapshot->primaryStart :
        (useNativePrimary ? primaryList->activeStart() :
                            state->subtitle()->startTime())) + primaryDelay);
    expression->setEndTime((mediaSnapshot != nullptr ?
        mediaSnapshot->primaryEnd :
        (useNativePrimary ? primaryList->activeEnd() :
                            state->subtitle()->endTime())) + primaryDelay);
    expression->setSubtitle2(mediaSnapshot != nullptr ?
        mediaSnapshot->secondaryText :
        (useNativeSecondary ? secondaryList->activeText() :
                              state->secondarySubtitle()->text()));
