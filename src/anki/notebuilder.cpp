 ...
 *
 * @param mediaSnapshot Immutable player state used by the encoder.
...
static bool createAudioMediaHelper(
    const Anki::Note::MediaSnapshot &mediaSnapshot,
    const AnkiProfile &profile,
    ...
    Anki::Note::Context &ctx)
{
    ...
    QString path = MpvController::encodeAudioClip(
        mediaSnapshot.encoding, args);
    if (path.isEmpty())
    {
        return false;
    }
...
/**
 * @brief Save an immutable in-memory frame to a temporary image file.
 */
[[nodiscard]]
static QString saveTemporaryImage(const QImage &image, const QString &ext)
{
    if (image.isNull())
    {
        return {};
    ...
    QTemporaryFile file(
        QDir::tempPath() + QStringLiteral("/memento-XXXXXX") + ext);
    if (!file.open())
    {
        return {};
    }
    ...
    if (format.isEmpty() || !image.save(&file, format.constData()))
    {
        return {};
    }
 ...
 *
 * @param mediaSnapshot Immutable player and subtitle state.
    ...
    Anki::Note::Context &ctx)
{
    ...
    const bool nativePrimary =
        args.subtitles && mediaSnapshot.completePrimaryTimeline;
    const bool nativeSecondary =
        args.subtitles && mediaSnapshot.completeSecondaryTimeline;
    if (args.subtitles &&
        (mediaSnapshot.completePrimaryTimeline ||
         mediaSnapshot.completeSecondaryTimeline))
    {
        if (mediaSnapshot.videoWidth <= 0 || mediaSnapshot.videoHeight <= 0)
        {
            return false;
        }
        SubtitleMedia::Track primaryTrack =
            mediaSnapshot.completePrimaryTimeline ?
                mediaSnapshot.primaryTrack : SubtitleMedia::Track{};
        SubtitleMedia::Track secondaryTrack =
            mediaSnapshot.completeSecondaryTimeline ?
                mediaSnapshot.secondaryTrack : SubtitleMedia::Track{};
        const bool primaryFallback =
            !mediaSnapshot.completePrimaryTimeline &&
            mediaSnapshot.encoding.primarySubtitle.valid();
        const bool secondaryFallback =
            !mediaSnapshot.completeSecondaryTimeline &&
            mediaSnapshot.encoding.secondarySubtitle.valid();
        const bool sharedDelayMixedRender =
            !mediaSnapshot.encoding.independentSecondaryDelay &&
            ((mediaSnapshot.completePrimaryTimeline && secondaryFallback) ||
             (mediaSnapshot.completeSecondaryTimeline && primaryFallback));
        if (sharedDelayMixedRender)
        {
            // mpv < 0.38 applies one delay to both subtitle slots. Leave the
            // generated timeline unshifted and let the encoder apply that
            // shared delay equally to it and the mpv-rendered fallback.
            primaryTrack.delay = 0.0;
            secondaryTrack.delay = 0.0;
        }
        const QByteArray ass = SubtitleMedia::makeAss(
            primaryTrack,
            secondaryTrack,
            mediaSnapshot.subtitleStyle,
            mediaSnapshot.videoWidth,
            mediaSnapshot.videoHeight,
            args.start,
            args.end
        );
        if (!ass.isEmpty())
        {
            QTemporaryFile subtitleFile(
                QDir::tempPath() + QStringLiteral("/memento-XXXXXX.ass"));
            if (!subtitleFile.open() ||
                subtitleFile.write(ass) != ass.size() || !subtitleFile.flush())
            {
                return false;
            }
            subtitleFile.setAutoRemove(false);
            subtitlePath = subtitleFile.fileName();
        }
    }

    QString path = MpvController::encodeVideoClip(
        mediaSnapshot.encoding,
        args,
        subtitlePath,
        nativePrimary,
        nativeSecondary
    ...
    Anki::Note::Context &ctx)
{
    ...
    {
        success = success &&
            createAudioMediaHelper(
                mediaSnapshot, profile, exp, params, fields, ctx);
    }
    return success;
}
    ...
    Anki::Note::Context &ctx)
{
    ...
    QImage image = mediaSnapshot.screenshotFrame;
    if (mediaSnapshot.nativePrimary || mediaSnapshot.nativeSecondary)
    {
        image = SubtitleMedia::renderOverlay(
            std::move(image),
            mediaSnapshot.nativePrimary ? mediaSnapshot.primaryText : QString(),
            mediaSnapshot.nativeSecondary ?
                mediaSnapshot.secondaryText : QString(),
            mediaSnapshot.subtitleStyle
        );
    }
    const QString path = saveTemporaryImage(image, imageExt);
    if (path.isEmpty())
    {
        return false;
    }
    ...
    {
        success = createScreenshotHelper(
            path, imageExt, params, fields, ctx) && success;
    }
 ...
 *
 * @param mediaSnapshot Immutable player state containing the video-only frame.
 * @param profile The profile to use when generating the context file.
 * @param fieldCtx The field context containing fields that include media.
 * @param[out] ctx The context to add the media to.
 * @return true if the media was added to the context,
    ...
    Anki::Note::Context &ctx)
{
    ...
    const QString path = saveTemporaryImage(
        mediaSnapshot.videoFrame, imageExt);
    if (path.isEmpty())
    {
        return false;
    }
...
Anki::Note::MediaSnapshot Anki::Note::captureMedia(
    const ::Context &appContext)
{
    MediaSnapshot snapshot;
    MpvPlayer *player = appContext.player();
    Settings *settings = appContext.settings();
    if (player == nullptr || player->state() == nullptr ||
        player->controller() == nullptr || settings == nullptr)
    {
        return snapshot;
    }

    MpvController *controller = player->controller();
    if (controller->thread() != QThread::currentThread())
    {
        qWarning("Anki media must be captured on the player thread");
        return snapshot;
    }

    MpvState *state = player->state();
    snapshot.encoding = controller->encodingContext();
    snapshot.subtitleStyle.font = settings->interfaceSubtitleFont();
    snapshot.subtitleStyle.textColor = settings->interfaceSubtitleColor();
    snapshot.subtitleStyle.backgroundColor =
        settings->interfaceSubtitleBackground();
    snapshot.subtitleStyle.strokeColor =
        settings->interfaceSubtitleStrokeColor();
    snapshot.subtitleStyle.scale = settings->interfaceSubtitleScale();
    snapshot.subtitleStyle.offset = settings->interfaceSubtitleOffset();
    snapshot.subtitleStyle.strokeWidth = settings->interfaceSubtitleStroke();
    snapshot.subtitleStyle.lineSpacing =
        settings->interfaceSubtitleLineSpacing();
    snapshot.subtitleStyle.removeRegex = settings->searchRemoveRegex();
    snapshot.subtitleStyle.replaceNewlines =
        settings->searchReplaceNewlines();
    snapshot.subtitleStyle.newlineReplacement =
        settings->searchReplaceNewlinesWith();

    const auto copyTrack = [] (
        const SubtitleListModel *model,
        const MpvSubtitle *subtitle,
        SubtitleMedia::Track &track)
    {
        if (subtitle != nullptr)
        {
            track.delay = subtitle->delay();
        }
        if (model == nullptr)
        {
            return;
        ...
        track.entries = model->items();
    };

    const SubtitleListModel *primary = appContext.subtitleLists()->primary();
    const SubtitleListModel *secondary =
        appContext.subtitleLists()->secondary();
    copyTrack(primary, state->subtitle(), snapshot.primaryTrack);
    copyTrack(secondary, state->secondarySubtitle(), snapshot.secondaryTrack);
    if (!snapshot.encoding.independentSecondaryDelay)
    {
        snapshot.secondaryTrack.delay = snapshot.encoding.sharedSubtitleDelay;
    }

    const auto knownTextTrackSelected = [state] (int64_t id) {
        return std::ranges::any_of(
            state->subtitleTracks(),
            [id] (const MpvTrack *track) {
                return track != nullptr && track->id() == id &&
                    !track->codec().trimmed().isEmpty() &&
                    !track->isBitmapSubtitle();
            }
        );
    };

    // Note media always uses Memento styling for selected text tracks,
    // independently of the live player's display preference. Bitmap tracks
    // remain explicit mpv fallbacks.
    snapshot.nativePrimary = state->sid() > 0 &&
        knownTextTrackSelected(state->sid());
    snapshot.nativeSecondary = state->secondarySid() > 0 &&
        knownTextTrackSelected(state->secondarySid());
    snapshot.completePrimaryTimeline = snapshot.nativePrimary &&
        primary != nullptr && primary->fullTimelineReady();
    snapshot.completeSecondaryTimeline = snapshot.nativeSecondary &&
        secondary != nullptr && secondary->fullTimelineReady();

    const bool fallbackPrimary = state->sid() > 0 && !snapshot.nativePrimary;
    const bool fallbackSecondary =
        state->secondarySid() > 0 && !snapshot.nativeSecondary;
    const MpvFrameCapture capture = controller->captureFrames(
        fallbackPrimary, fallbackSecondary);
    snapshot.stateCaptured = capture.stateCaptured;
    snapshot.position = capture.stateCaptured ?
        capture.position : state->timePosition();
    ...
    const auto activeCueAt = [] (
        const std::vector<SubtitleEntry> &entries, double position)
    {
        ...
        {
            if (entry.start < position + TIME_DELTA && position < entry.end)
            {
                text.append(entry.text);
                cue.start = text.size() == 1 ?
                    entry.start : std::min(cue.start, entry.start);
                cue.end = text.size() == 1 ?
                    entry.end : std::max(cue.end, entry.end);
            }
        }
        cue.text = text.join(QLatin1Char('\n'));
        return cue;
    };

    const ActiveCue primaryCue = snapshot.completePrimaryTimeline ?
        activeCueAt(
            snapshot.primaryTrack.entries,
            snapshot.position - snapshot.primaryTrack.delay) : ActiveCue{};
    const ActiveCue secondaryCue = snapshot.completeSecondaryTimeline ?
        activeCueAt(
            snapshot.secondaryTrack.entries,
            snapshot.position - snapshot.secondaryTrack.delay) : ActiveCue{};
    snapshot.primaryText = snapshot.completePrimaryTimeline ?
        primaryCue.text : (capture.stateCaptured ?
            capture.primaryText : state->subtitle()->text());
    snapshot.secondaryText = snapshot.completeSecondaryTimeline ?
        secondaryCue.text : (capture.stateCaptured ?
            capture.secondaryText : state->secondarySubtitle()->text());
    snapshot.primaryStart = snapshot.completePrimaryTimeline ?
        primaryCue.start : (capture.stateCaptured ?
            capture.primaryStart : state->subtitle()->startTime());
    snapshot.primaryEnd = snapshot.completePrimaryTimeline ?
        primaryCue.end : (capture.stateCaptured ?
            capture.primaryEnd : state->subtitle()->endTime());
    snapshot.secondaryStart = snapshot.completeSecondaryTimeline ?
        secondaryCue.start : (capture.stateCaptured ?
            capture.secondaryStart : state->secondarySubtitle()->startTime());
    snapshot.secondaryEnd = snapshot.completeSecondaryTimeline ?
        secondaryCue.end : (capture.stateCaptured ?
            capture.secondaryEnd : state->secondarySubtitle()->endTime());
    snapshot.primaryTrack.activeText = snapshot.primaryText;
    snapshot.secondaryTrack.activeText = snapshot.secondaryText;

    snapshot.videoFrame = capture.videoFrame;
    snapshot.videoWidth = snapshot.videoFrame.isNull() ?
        static_cast<int>(state->videoWidth()) : snapshot.videoFrame.width();
    snapshot.videoHeight = snapshot.videoFrame.isNull() ?
        static_cast<int>(state->videoHeight()) : snapshot.videoFrame.height();
    snapshot.screenshotFrame = capture.subtitleFrame;
    return snapshot;
}
    ...
    const MediaSnapshot &mediaSnapshot)
{
    ...
    if (media)
    {
        ...
        createAudioMedia(mediaSnapshot, profile, term, fieldCtx, ctx);
        createScreenshot(mediaSnapshot, profile, fieldCtx, ctx);
        createScreenshotVideo(mediaSnapshot, profile, fieldCtx, ctx);
        createVideo(mediaSnapshot, profile, term, fieldCtx, ctx);
    ...
    const MediaSnapshot &mediaSnapshot)
{
    ...
    if (media)
    {
        createAudioMedia(mediaSnapshot, profile, kanji, fieldCtx, ctx);
        createScreenshot(mediaSnapshot, profile, fieldCtx, ctx);
        createScreenshotVideo(mediaSnapshot, profile, fieldCtx, ctx);
        createVideo(mediaSnapshot, profile, kanji, fieldCtx, ctx);
