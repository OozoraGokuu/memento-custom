...
void MpvPlayer::initPropertyMap()
{
    ...
    m_propertyMap["sub-delay"] = [this] (mpv_event_property *prop)
    {
        if (prop->format == MPV_FORMAT_DOUBLE)
        {
            ...
            {
                // Before mpv 0.38 both subtitle slots shared sub-delay.
...
void MpvPlayer::handleMpvEvent(mpv_event *event)
{
    ...
    case MPV_EVENT_PROPERTY_CHANGE:
    {
        ...
        if (QByteArray(prop->name) == "paused-for-cache" && prop->data) {
    ...
    case MPV_EVENT_END_FILE:
    {
        ...
        if (end && end->reason == MPV_END_FILE_REASON_ERROR)
            m_playbackError = tr("Could not load episode: %1").arg(QString::fromUtf8(mpv_error_string(end->error)));
        emit playbackStatusChanged();
        break;
    }
