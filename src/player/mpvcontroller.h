...
/**
 * @brief Immutable values needed by a standalone mpv encoder.
 *
 * Instances are captured from the live player on its owning thread and may
 * then be copied to a worker. No QObject or mpv handle is retained.
 */
struct MpvEncodingContext
{
    ...
    QByteArray input;
    QByteArray scriptOptions;
    int64_t aid{-1};
    int64_t vid{-1};
    double sharedSubtitleDelay{0.0};
    bool independentSecondaryDelay{false};
    SubtitleTrack primarySubtitle;
    SubtitleTrack secondarySubtitle;
};

/**
 * @brief A pair of screenshots and subtitle state captured at one paused frame.
 *
 * The live player's prior pause and subtitle-visibility values are restored
 * before this value is returned.
 */
struct MpvFrameCapture
{
    QImage videoFrame;
    QImage subtitleFrame;
    QString primaryText;
...
public:
    ...
    /**
     * @brief Capture a subtitle screenshot with explicit per-slot visibility.
     *
     * Visibility is changed synchronously only for the duration of the
     * screenshot and restored before returning.
     *
     * @param primary Whether mpv should render the primary subtitle slot.
     * @param secondary Whether mpv should render the secondary subtitle slot.
     * @return The screenshot image, or a null image on failure.
     */
    [[nodiscard]]
    QImage screenshotRawWithSubtitleVisibility(bool primary, bool secondary);

    /**
     * @brief Atomically capture video, requested mpv subtitles, and cue state.
     *
     * Playback is synchronously paused for the two screenshots, then its exact
     * prior pause and per-slot visibility values are restored.
     */
    [[nodiscard]] MpvFrameCapture captureFrames(
    ...
    /**
     * @brief Capture all live player values needed by standalone encoders.
     *
     * This must be called on the controller's owning thread.
     */
    [[nodiscard]]
    MpvEncodingContext encodingContext() const;

    /**
     * @brief Encode an audio clip from an immutable player snapshot.
     */
    [[nodiscard]]
    static QString encodeAudioClip(
        const MpvEncodingContext &context,
        const MpvAudioClipArgs &args);

    /**
     * @brief Snapshot an audio clip job that is safe to execute on a worker.
     *
     * All player and mpv state needed by the encoder is copied before this
     * method returns. The returned callable never accesses this controller.
     *
     * @param args The arguments used to create the audio clip.
     * @return A self-contained encoder task, or an empty callable on failure.
     */
    [[nodiscard]]
    std::function<QString()> makeTempAudioClipTask(
    ...
    /**
     * @brief Encode a video clip from an immutable player snapshot.
     *
     * When subtitleFile is non-empty it is a generated subtitle track. The
     * nativePrimary/nativeSecondary flags describe which selected tracks it
     * replaces; any other selected track remains available as an mpv fallback
     * (for example PGS or VobSub bitmap subtitles).
     */
    [[nodiscard]]
    static QString encodeVideoClip(
        const MpvEncodingContext &context,
...
private:
     ...
     */
    [[nodiscard]]
    static QString encodeFile(
        const QByteArray &input,
        const QByteArray &scriptOptions,
        const QByteArray &argString,
        const QList<QPair<QByteArray, QByteArray>> &options,
        const QString &fileExtension,
        const MpvEncodingContext *subtitleContext = nullptr,
        const QString &generatedSubtitleFile = {},
        bool nativePrimary = false,
        bool nativeSecondary = false);

    /**
     * @brief Snapshot script options required by a standalone encoder handle.
     */
    [[nodiscard]]
    QByteArray encodingScriptOptions() const;
