...
void MpvTrack::setCodec(QString value)
{
    ...
    const bool wasBitmapSubtitle = isBitmapSubtitle();
    m_codec = std::move(value);
    emit codecChanged(m_codec);

    const bool bitmapSubtitle = isBitmapSubtitle();
    if (wasBitmapSubtitle != bitmapSubtitle)
    {
        emit bitmapSubtitleChanged(bitmapSubtitle);
    }
}

bool MpvTrack::isBitmapSubtitleCodec(QStringView codec)
{
    QString normalized = codec.trimmed().toString().toLower();
    normalized.removeIf([](QChar character) {
        return character == u'_' || character == u'-' ||
               character == u' ' || character == u'/';
    });

    return normalized == QStringLiteral("hdmvpgssubtitle") ||
           normalized == QStringLiteral("hdmvpgs") ||
           normalized == QStringLiteral("pgs") ||
           normalized == QStringLiteral("pgssub") ||
           normalized == QStringLiteral("dvdsubtitle") ||
           normalized == QStringLiteral("dvdsub") ||
           normalized == QStringLiteral("vobsubtitle") ||
           normalized == QStringLiteral("vobsub") ||
           normalized == QStringLiteral("idxsub") ||
           normalized == QStringLiteral("dvbsubtitle") ||
           normalized == QStringLiteral("dvbsub") ||
           normalized == QStringLiteral("xsub");
}

bool MpvTrack::isBitmapSubtitle() const
{
    return isBitmapSubtitleCodec(m_codec);
}
