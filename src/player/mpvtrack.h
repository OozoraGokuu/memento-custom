...
class MpvTrack : public QObject
{
    ...
    Q_PROPERTY(
        bool bitmapSubtitle
        READ isBitmapSubtitle
        NOTIFY bitmapSubtitleChanged
    )
...
public:
    ...
    /**
     * @brief Tests whether a codec identifies a bitmap subtitle stream.
     *
     * Codec names are matched case-insensitively and common separator
     * variations used for PGS, VobSub/IDX-SUB, DVB, and XSUB are accepted.
     *
     * @param codec The codec name to classify.
     * @return true if mpv must render the subtitle as a bitmap,
     * @return false if it can be rendered as text.
     */
    [[nodiscard]]
    static bool isBitmapSubtitleCodec(QStringView codec);

    /**
     * @brief Whether this track contains bitmap subtitles.
     *
     * @return true if mpv must render this subtitle track,
     * @return false if it can be rendered as text.
     */
    [[nodiscard]]
    bool isBitmapSubtitle() const;
...
signals:
    ...
    /**
     * @brief Emitted when the codec changes between text and bitmap types.
     *
     * @param value True if the track contains bitmap subtitles.
     */
    void bitmapSubtitleChanged(bool value);
