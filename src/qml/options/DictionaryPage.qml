...
Page {
    ...
    header: ColumnLayout {
        ...
        ToolBar {
            ...
            RowLayout {
                ...
                Button {
                    text: qsTr("Install default dictionaries")
                    enabled: !DictionaryController.busy && !DictionaryController.modifyingDatabase
                    onClicked: DictionaryController.installDefaults()
                }
                ToolButton {
    ...
    ColumnLayout {
        ...
        Label {
            ...
            Layout.margins: 10
            wrapMode: Text.Wrap
            text: qsTr("Defaults: Jitendex for Japanese–English definitions and JPDB for word frequency. Downloaded from their public sources; your own dictionaries are preserved.")
        }
        Label {
            Layout.fillWidth: true
            Layout.margins: 10
            ...
            text: DictionaryController.modifyingDatabase && !DictionaryController.busy ? qsTr("Updating dictionaries…") : DictionaryController.activity
        }
        ProgressBar {
            Layout.fillWidth: true
            visible: DictionaryController.busy || DictionaryController.modifyingDatabase
            indeterminate: !DictionaryController.busy || DictionaryController.progress < 0
            value: Math.max(0, DictionaryController.progress)
        }
        Button {
            visible: DictionaryController.busy
            text: DictionaryController.modifyingDatabase ? qsTr("Stop after current import") : qsTr("Cancel downloads")
            onClicked: DictionaryController.cancelPendingImports()
        }
        ListView {
            id: listView
            Layout.fillWidth: true
