...
ApplicationWindow {
    ...
    SplitView {
        ...
        Player {
            ...
            onFileLoaded: function(w, h) {
                ...
                if (player.state.path.length > 0 &&
                    !EpisodeLibrary.isTransientStream(player.state.path))
                {
        ...
        SplitView {
            ...
            SplitView.minimumWidth: toolSplitView.hasVisibleChildren ? 340 : 0
    ...
    TorrentSearchDialog {
        id: torrentSearchDialog
        player: player
    }

    JimakuSearchDialog {
        id: jimakuSearchDialog
        player: player
    }
