...
ListView {
    ...
    /**
     * Emitted when a subtitle should start at the current player position.
     * @param start The original start time of the subtitle.
     */
    signal startFromSubtitleRequested(start: real)
    ...
    Menu {
        ...
        Action {
            text: qsTr("Start from this subtitle")
            enabled: subtitleContextMenu.subtitleIndex >= 0
            onTriggered: {
                const modelIndex = root.subtitleListModel.index(
                    subtitleContextMenu.subtitleIndex, 0);
                const start = root.subtitleListModel.data(
                    modelIndex, SubtitleListModel.StartRole);
                root.startFromSubtitleRequested(start);
            }
        }
    }
        ...
        onPressed: function(event) {
            ...
            if (event.button === Qt.RightButton)
            {
                ...
                root.selectionModel.setCurrentIndex(
                    modelIndex, ItemSelectionModel.NoUpdate);
                subtitleContextMenu.subtitleIndex = index;
