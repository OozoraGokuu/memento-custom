                ...
                MouseArea {
                    ...
                    onPositionChanged: function(mouse) {
                        ...
                        if (x >= 0 && x < textEdit.width)
                        {
                            ...
                            if (textMouseArea.leftPressed &&
                                textMouseArea.pressIndex >= 0 &&
                                index !== textMouseArea.pressIndex)
                            {
                                textMouseArea.selecting = true;
                                root.select(
                                    Math.min(textMouseArea.pressIndex, index),
                                    Math.max(textMouseArea.pressIndex, index) + 1
                                );
                            }
                        }
                    }
                    onPressed: function(event) {
                        ...
                        const x = event.x - root.margin;
                        textMouseArea.pressIndex =
                            x >= 0 && x < textEdit.width ?
                                textEdit.getCharacterIndexAt(x) : -1;
                        textMouseArea.selecting = false;
                        textMouseArea.leftPressed = true;
                    }
                    onClicked: function(event) {
                        if (event.button === Qt.LeftButton)
                        {
                            if (!textMouseArea.selecting)
                            {
                                root.clicked();
                            }
                        }
                    ...
                    onReleased: function(event) {
                        if (event.button === Qt.LeftButton &&
                            textMouseArea.selecting &&
                            root.selectedText.length > 0)
                        {
                            root.selectionFinished();
                        }
                        textMouseArea.pressIndex = -1;
                        textMouseArea.leftPressed = false;
                    }
                    onCanceled: {
                        textMouseArea.pressIndex = -1;
                        textMouseArea.leftPressed = false;
