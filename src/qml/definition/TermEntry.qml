...
Item {
    ...
    /**
     * Save the current screenshot and sentence audio to the chosen folder.
     */
    function sendToMigaku()
    {
        if (!MigakuClient.enabled || !root.term || MigakuClient.busy)
        {
            return;
        }

        MigakuClient.exportCurrentSubtitle().then(function(result) {
            if (!result.success)
            {
                errorDialog.title = qsTr("Could Not Prepare Migaku Export");
                errorLabel.text = result.error;
                errorDialog.open();
            }
        });
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    ToolButton {
                        id: sendMigakuButton
                        visible: MigakuClient.enabled

                        focusPolicy: Qt.NoFocus
                        enabled: root.term && !MigakuClient.busy
                        text: MigakuClient.busy ? qsTr("Saving…") : qsTr("Migaku")
                        display: AbstractButton.TextOnly
                        font.bold: true
                        onClicked: root.sendToMigaku()

                        ToolTip.visible: hovered
                        ToolTip.text: qsTr(
                            "Save the screenshot and " +
                            "sentence audio to your Migaku export folder"
                        )
                    }
