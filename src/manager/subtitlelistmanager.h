...
private slots:
    /**
     * @brief Reset subtitle history for a newly loaded media file.
     */
    ...
    /**
     * @brief Update both active subtitle models for the player position.
     *
     * @param position The current player position without subtitle delay.
     */
    void handleTimePositionChanged(double position);

    /**
     * @brief Re-evaluate the primary subtitle after its delay changes.
     *
     * @param delay The new primary subtitle delay.
     */
    void handlePrimaryDelayChanged(double delay);

    /**
     * @brief Re-evaluate the secondary subtitle after its delay changes.
     *
     * @param delay The new secondary subtitle delay.
     */
    void handleSecondaryDelayChanged(double delay);
    ...
    /**
     * @brief Select the subtitle active at the delayed player position.
     *
     * @param model The model to update.
     * @param subtitle The subtitle state containing the track delay.
     * @param position The current player position without delay.
     */
    void selectPosition(
        SubtitleListModel *model,
...
private:
    /**
     * @brief A subtitle model and the track identity needed to reuse it safely.
     */
    struct TrackModel
    {
    ...
    /* Subtitle list models keyed by mpv track ID */
    QHash<int64_t, TrackModel> m_models;

    /* Media path associated with m_models */
    QString m_mediaPath;
};
